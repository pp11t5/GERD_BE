...
- 공용 skill 원본은 `shared/backend-core-skills` 에 `git subtree` 로 포함합니다.
- `.claude/skills`, `.agents/skills` 는 subtree 내부 경로를 가리키는 symlink입니다.
- 공용 저장소 최신 변경을 가져올 때:

```bash
git subtree pull --prefix=shared/backend-core-skills backend-core-skills main --squash
```

- 현재 저장소 변경을 공용 저장소로 보낼 때:

```bash
git subtree push --prefix=shared/backend-core-skills backend-core-skills main
```
