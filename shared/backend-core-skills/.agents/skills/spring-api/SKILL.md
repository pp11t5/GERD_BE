...
- `ResponseEntity<ApiResponse<T>>`를 쓰더라도 인터페이스 반환 타입에 구체 `T`를 노출한다. `ApiResponse<?>` 같은 와일드카드 타입은 계약을 흐리므로 기본값으로 쓰지 않는다.
...
- Service에 위임하고 성공 응답 조립만 수행한다.
- `ResponseEntity`가 꼭 필요하지 않으면 `ApiResponse<T>`를 직접 반환한다.
- `ResponseEntity`가 필요하면 Controller에서 직접 `ResponseEntity.status(...).body(ApiResponse.onSuccess(...))`를 작성한다.
- 작은 프로젝트나 단순 API에서는 `ApiResponseEntity` 같은 별도 성공 응답 헬퍼를 두지 않고 Controller에서 직접 반환하는 쪽을 우선한다.
...
상태코드 제어가 필요한 경우는 아래처럼 작성한다.

```kotlin
// [Domain]Api.kt
@PostMapping("/[domains]")
fun create[Domain](
    @Valid @RequestBody request: Create[Domain]RequestDTO,
): ResponseEntity<ApiResponse<Create[Domain]ResponseDTO>>

// [Domain]Controller.kt
override fun create[Domain](
    @Valid @RequestBody request: Create[Domain]RequestDTO,
): ResponseEntity<ApiResponse<Create[Domain]ResponseDTO>> =
    ResponseEntity
        .status(CommonSuccessCode.CREATED.httpStatus)
        .body(ApiResponse.onSuccess([domain]Service.create(request), CommonSuccessCode.CREATED))
```
...
- 설명은 한 줄 요약으로 끝내지 않고 요청값, 내부 처리 흐름, 성공 결과, 주요 실패 조건 순으로 구체적으로 작성한다.
- 설명은 가능하면 Kotlin raw string `""" ... """`과 `-` bullet 형식을 사용해 읽기 좋게 작성한다.
- request body의 어떤 필드를 사용하는지, 중요한 검증 조건이 무엇인지, 재발급/상태 변경 같은 부수효과가 있는지 설명에 드러낸다.
- 요청/응답 DTO의 각 필드 의미가 중요하면 `@Schema(description = ..., example = ...)`를 DTO 필드에 추가한다.
- 필드별 스키마 설명을 아직 붙이지 못한 경우에도 최소한 `@Operation.description`에는 어떤 필드를 받고 무엇을 반환하는지 드러낸다.
- 설명에는 현재 구현된 규칙만 적는다. 아직 구현되지 않은 `deviceId` 검증, rotation 무효화, 헤더 요구사항 같은 조건을 추정해서 문서화하지 않는다.
...
## `@Valid` / `@Validated` 사용 기준

- `@RequestBody` DTO 검증은 기본적으로 Controller 메서드 파라미터에 `@Valid`를 붙인다.
- `@PathVariable`, `@RequestParam`, `@ModelAttribute`에 선언한 `@Min`, `@Positive`, `@NotBlank` 같은 제약을 활성화하려면 Controller 클래스나 인터페이스에 `@Validated`를 붙인다.
- 단건 요청 DTO 검증만 있으면 `@Validated`를 습관적으로 추가하지 않는다.
- `@Validated`를 붙였다면 실패 예외는 주로 `ConstraintViolationException`으로 들어오므로 전역 핸들러와의 연결을 같이 확인한다.
...
class PostController(
    ...
    override fun getPost(
        @PathVariable @Positive(message = "게시글 ID는 1 이상이어야 합니다") postId: Long,
    ): ApiResponse<PostResponseDTO> =
        ApiResponse.onSuccess(postQueryService.get(postId))
}
```
...
- Body DTO 검증: `@Valid`
- Path/Query 파라미터 제약 검증 활성화: `@Validated`
...
- `ResponseEntity`는 생성, 비동기 접수, 특수 헤더, status 분기처럼 실제 제어가 필요할 때만 사용한다.
- 단순 조회/수정처럼 `200 OK`면 `ApiResponse<T>` 직접 반환을 우선한다.
- `ResponseEntity<ApiResponse<T>>`를 사용할 때는 body의 success code와 HTTP status가 같은 `BaseSuccessCode`에서 나오도록 맞춘다.
- `204 No Content`는 현재 프로젝트의 공통 `ApiResponse` body 정책과 충돌하므로 기본 선택지로 두지 않는다.
...
- `@Validated`가 필요한 파라미터 검증인데 빠지지 않았는가?
- `ResponseEntity`를 썼다면 실제 status와 `ApiResponse` success code가 일치하는가?
- 전역 에러 핸들링과 충돌하지 않는가?
