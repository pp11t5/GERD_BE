...
private val CONSTRAINT_NAME_REGEX = Regex("constraint \"([^\"]+)\"")
...
class GlobalExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    fun handleDataIntegrityViolation(e: DataIntegrityViolationException, request: WebRequest): ResponseEntity<Any> {
        ...
        // 제약조건 이름만 로그로 남기고 "Detail: Key (col)=(value) ..." 등 실제 컬럼 값은 제외
        val constraintName = CONSTRAINT_NAME_REGEX.find(cause)?.groupValues?.get(1) ?: "unknown"
        ...
        if (cause.contains("foreign key", ignoreCase = true)) {
            log.error("DataIntegrityViolationException(FK): constraint={}", constraintName)
        } else {
