...
interface FoodCategoryMapRepository : JpaRepository<FoodCategoryMap, FoodCategoryMapId> {
    ...
    /**
     * 여러 음식의 카테고리 code를 한 번에 로딩 (검색/최근 응답 조립 시 N+1 회피)
     *
     * foodId별 그룹핑은 서비스에서 수행. sortOrder 순으로 정렬해 표시 순서를 보장한다.
     */
    @Query(
        """
        select m.food.id as foodId, c.code as code
...
// 음식↔카테고리 code 평면 프로젝션 (foodId 기준 그룹핑용)
